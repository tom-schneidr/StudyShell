...
export default function FlashcardDeck({ cards, onClose, title = "Flashcard Session" }: FlashcardDeckProps) {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-shell-bg/95 backdrop-blur-xl p-8">
        <div className="text-center">
          ...
          <button onClick={onClose} className="px-6 py-2 rounded-xl bg-shell-surface hover:bg-shell-surface-hover transition-colors">Close</button>
        </div>
      </div>
    );
  }
        ...
        <div className="w-full max-w-4xl flex flex-col h-full relative z-10">
            ...
            <header className="flex items-center justify-between mb-12">
                <div className="flex flex-col">
                    <h2 className="text-2xl font-black text-shell-text tracking-tight uppercase">{title}</h2>
                  ...
                  {!completed ? (
                                ...
                                {cards.map((_, i) => (
                                    <div 
                                      ...
                                      className={`h-1.5 rounded-full transition-all duration-300 ${
                                          i === currentIndex ? "w-8 bg-shell-accent" : i < currentIndex ? "w-2 bg-emerald-400" : "w-2 bg-shell-border"
                                      }`}
                                    />
                                ))}
                            </div>
                    ...
                    >
                        ...
                        <p className="text-shell-text-secondary leading-relaxed mb-10">
                            You've reviewed all {cards.length} cards in this deck. Great job keeping your study streak alive.
                        </p>
